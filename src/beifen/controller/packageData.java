...
public class packageData {
    ...
    public String selectMachineInfo (String sql){
        ...
        try {
            ret = db1.pst.executeQuery();//执行语句，得到结果集
            while (ret.next()) {
                machineInfo = ret.getString(1)+"|"+ret.getString(2)+"|"+ret.getString(3)+"|"+ret.getString(4)+"|"+ret.getString(5)+"|"+ret.getString(6)+"|"+ret.getString(7);
            }
            ret.close();
            db1.close();//关闭连接
        } catch (SQLException e) {
    ...
    //生成二维码
    public void apkEncode(String filename,String filePath,int buildNumber,String url) throws WriterException, IOException {
